...
void SystemThread::WarpVehicle(int targetVID, float xTo, float yTo, float dirTo)
{
    ...
    // 添加边界检查，确保车辆不会被传送到边界外
    if (xTo < -550.0 || xTo > 550.0 || yTo < -350.0 || yTo > 350.0) {
        qDebug() << "警告: 目标位置超出边界范围，拒绝传送车辆: ID=" << targetVID 
                 << " x=" << xTo << " y=" << yTo;
        return;
    }
...
void SystemThread::WarpVehicleAdjustPosToLane(int targetVID, float xTo, float yTo, float dirTo)
{
    ...
    if( agent[targetVID]->agentKind < 100 ){
        ...
        // 添加边界检查，确保车辆不会被调整到边界外
        if (xTo < -550.0 || xTo > 550.0 || yTo < -350.0 || yTo > 350.0) {
            qDebug() << "警告: 调整后的位置超出边界范围，拒绝传送车辆: ID=" << targetVID 
                     << " x=" << xTo << " y=" << yTo;
            return;
        }
