    ...
    QThread(parent)
{
    ...
    sysFile   = QString();
    confFile  = QString();
    qDebug() << "SystemThread构造函数中设置sysFile = " << sysFile;
    udpThread = NULL;
...
//                qDebug() << "sendSize = " << sendSize;
        ...
        for(int i=0;i<maxAgent;++i){
            // 在更新前先检查车辆状态
            if (agent[i]->agentStatus == 2) {
                // 对待处理的车辆，确保完全清理干净
                agent[i]->agentStatus = 0;
                agent[i]->TimeOfAppear = -1.0;
                agent[i]->isScenarioObject = false;
                agent[i]->isOldScenarioType = false;
                agent[i]->skipSetControlInfo = false;
                continue; // 跳过待处理的车辆
            }
            
            // 检查位置是否已经超出边界
            if (agent[i]->agentStatus == 1 &&
                (agent[i]->state.x < -550.0 || agent[i]->state.x > 550.0 || 
                 agent[i]->state.y < -350.0 || agent[i]->state.y > 350.0)) {
                qDebug() << "车辆位置已超出边界，标记待处理: ID=" << i 
                ...
                agent[i]->isOldScenarioType = false;
                agent[i]->skipSetControlInfo = false;
                agent[i]->memory.currentTargetPath = -1;
                agent[i]->memory.targetPathList.clear();
                
                // 停止运动
                agent[i]->state.V = 0.0;
                if (agent[i]->agentKind < 100) {
                    agent[i]->vehicle.input.accel = 0.0;
                    agent[i]->vehicle.input.brake = 1.0;
                }
                continue; // 跳过更新
            }
            ...
            // 更新后检查状态，如果超出边界，标记为待处理
            if (agent[i]->agentStatus == 1 &&
                (agent[i]->state.x < -550.0 || agent[i]->state.x > 550.0 || 
                 agent[i]->state.y < -350.0 || agent[i]->state.y > 350.0)) {
                qDebug() << "更新后车辆超出边界，标记待处理: ID=" << i 
                         << " x=" << agent[i]->state.x << " y=" << agent[i]->state.y;
            ...
            // 更新后再次检查agentStatus状态，如果变为待处理，则立即清理
            else if (agent[i]->agentStatus == 2) {
                qDebug() << "立即处理待销毁车辆: ID=" << i;
...
void SystemThread::SetSysFile(QString filename)
{
    ...
    qDebug() << "sysFile已加载，检查UDP通信状态:";
    
    // 自动启用DS模式，确保UDP命令能够被处理
    //SetDSmode();
    //qDebug() << "DS模式已自动启用，UDP命令处理已激活";
    ...
    // 设置测试消息前缀和内容
    strcpy(testBuf, "TEST_MESSAGE_FROM_RESIM");
    
    // 发送到本地Python监听端口
    if(udpThread) {
        udpThread->SendScenarioData("127.0.0.1", 20000, testBuf, strlen(testBuf) + 1);
        qDebug() << "已向Python (127.0.0.1:20000) 发送测试消息: " << testBuf;
    }
}
...
//    qDebug() << "S-Interface Object: near path = " << pathId;
    ...
    if( pathId < 0 ){
        ...
        // ... existing code for pathId >= 0 case ...
    }
}
