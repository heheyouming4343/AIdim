...
//            qDebug() << " cPos=" << cPos;
            ...
            if( agent[cIdx]->agentKind < 100 ){
                ...
                if( i >= 0 && i < road->odRoute.size() &&
                    ...
                    if(selIdx >= 0 && selIdx < road->odRoute[i]->mergeLanesInfo.size()) {
                        ...
                        qDebug() << "Agent[" << agent[cIdx]->ID << "]: selIdx超出mergeLanesInfo范围: " << selIdx 
                                 << ", 最大值: " << (road->odRoute[i]->mergeLanesInfo.size() - 1);
                    }
                }
                else{  // new ver
                    ...
                    if( i >= 0 && i < road->odRoute.size() &&
                        ...
                        if( j >= 0 && j < road->odRoute[i]->LCSupportLaneLists.size() &&
                            ...
                            if(MLIidx >= 0 && MLIidx < road->odRoute[i]->mergeLanesInfo.size()) {
                                ...
                                qDebug() << "Agent[" << agent[cIdx]->ID << "]: MLIidx超出mergeLanesInfo范围: " << MLIidx 
                                         << ", 最大值: " << (road->odRoute[i]->mergeLanesInfo.size() - 1);
                            }
                            ...
                            for(int l=0;l<validRoutes.size();++l){
                                if( QString(validRoutes[l]).startsWith(QString("%1,").arg(rIdx[k])) == true ){
                                    if( m == k ){
                                        ...
                                        if(MLIidx >= 0 && MLIidx < road->odRoute[kk]->mergeLanesInfo.size()) {
                                            ...
                                            qDebug() << "Agent[" << agent[cIdx]->ID << "]: MLIidx超出mergeLanesInfo范围: " << MLIidx 
                                                     << ", 最大值: " << (road->odRoute[kk]->mergeLanesInfo.size() - 1);
                                        }
