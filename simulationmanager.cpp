        ...
        if( pAgent[objectID]->agentKind < 100 ){
            ...
            if( se->eventIntData[2] == 0 ){
                ...
                if( pRoad->odRoute[i]->LCSupportLaneLists.size() > 0 ){    // new version
                    ...
                    pAgent[objectID]->memory.targetPathList = pRoad->odRoute[i]->laneListsToDestination[selIdx];
                    pAgent[objectID]->memory.currentTargetPath = pAgent[objectID]->memory.targetPathList.last();
