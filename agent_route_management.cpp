...
//            qDebug() << "justWarped = " << justWarped;
            ...
            if( memory.targetPathList.indexOf( memory.currentTargetPath ) == 0 ){
                ...
                if( currentPath < 0 ){
                    ...
                    agentStatus = 2;
                    state.V = 0.0;
                    vehicle.input.accel = 0.0;
                    vehicle.input.brake = 1.0;
                    return;
            ...
            if( memory.targetPathList.indexOf( memory.currentTargetPath ) < 0 ){
                ...
                if( pRoad->odRoute[i]->LCSupportLaneLists.size() > 0 ){    // new version
                    ...
                    for(int j=0;j<pRoad->odRoute[i]->LCSupportLaneLists.size();++j){
                        ...
                        if(MLIidx >= 0 && MLIidx < pRoad->odRoute[i]->mergeLanesInfo.size()) {
                            ...
                            qDebug() << "Agent[" << ID << "]: MLIidx超出mergeLanesInfo范围: " << MLIidx 
                                     << ", 最大值: " << (pRoad->odRoute[i]->mergeLanesInfo.size() - 1);
                        }
...
//    qDebug() << "[SetTargetNodeListByTargetPaths] ID = " << ID;
    ...
    for(int i=0;i<memory.myNodeList.size()-1;++i){
        if(i >= 0 && i+1 < memory.myNodeList.size() && i < memory.myInDirList.size()) {
    ...
    for(int i=0;i<memory.myNodeList.size()-1;++i){
        if(i >= 0 && i < memory.myNodeList.size() && i < memory.myInDirList.size() && i < memory.myOutDirList.size()) {
    ...
    for(int i=0;i<memory.myNodeList.size();++i){
        if(i >= 0 && i < memory.myNodeList.size() && i < memory.myInDirList.size()) {
            ...
            if( memory.currentTargetNodeIndexInNodeList >= 0 ){
                if(i < memory.myTurnDirectionList.size() && 
                   (memory.myTurnDirectionList.at(i) == DIRECTION_LABEL::LEFT_CROSSING ||
...
//                                     << " InDir = " << pRoad->paths[cpIdx]->connectingNodeInDir;
    ...
    for(int i=0;i<memory.oncomingWaitPathList.size();++i){
        if(i < memory.oncomingWaitCPList.size()) {
            float dist = 0;
            for(int j=memory.currentTargetPathIndexInList;j>=0;j--){
